"""
TDSRegWorker - Multi-threading worker cho TDS (Traodoisub) account registration
Chạy registration tài khoản TDS trong thread riêng
"""

from PyQt5.QtCore import QThread, pyqtSignal


class TDSRegWorker(QThread):
    """Tạo 1 TDS account trong thread riêng."""
    ...
    def run(self):
        from TDS.REG import RegTDS
        
        try:
            self.log_msg.emit(f"[→] Worker {self.worker_id}: Bắt đầu tạo TDS account...")
            reg = RegTDS()
            result = reg.run()
...
class TDSRegBatchManager:
    """Quản lý pool thread để tạo nhiều TDS account cùng lúc."""
...
class CheckXuWorker(QThread):
    """Kiểm tra XU cho 1 tài khoản TDS trong thread riêng."""
